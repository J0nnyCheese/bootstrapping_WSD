...
class WSDInstance:
    ...
    def __str__(self):
        return '%s\t%s\t%s\t%d' % (self.id, self.lemma, ' '.join(self.context).decode(), self.index)
...
if __name__ == '__main__':
    ...
    # for each pair of (k,v) in dev_instances, if key := k is in dev_key, then add k:v into dev_instances
